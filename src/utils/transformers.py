...
class AttentionCrossAttention(Attention):
    ...
    def forward(self, x, x2 = None, use_attn=True):
        B, N, C = x.shape
        if x2 is None:
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q, k, v = qkv[0], qkv[1], qkv[2]
...
#     def forward(self, src: torch.Tensor, tgt: torch.Tensor = None, src_tgt: torch.Tensor = None, *args, **kwargs) -> torch.Tensor:
#         if src is not None and tgt is not None:
#             x = self.drop_path(self.self_attn(self.pre_norm(src), self.pre_norm(tgt)))
#             if src_tgt is not None: 
#                 x = x + src_tgt
#             x = self.norm1(x)
#             x2 = self.linear2(self.dropout1(self.activation(self.linear1(x))))
#             src_tgt = x + self.drop_path(self.dropout2(x2))

#             x = src + self.drop_path(self.self_attn(self.pre_norm(src)))
#             x = self.norm1(x)
#             x2 = self.linear2(self.dropout1(self.activation(self.linear1(x))))
#             src = x + self.drop_path(self.dropout2(x2))

#             x = tgt + self.drop_path(self.self_attn(self.pre_norm(tgt)))
#             x = self.norm1(x)
#             x2 = self.linear2(self.dropout1(self.activation(self.linear1(x))))
#             tgt = x + self.drop_path(self.dropout2(x2))

#             return src, tgt, src_tgt
#         else:
#             x = src + self.drop_path(self.self_attn(self.pre_norm(src)))
#             x = self.norm1(x)
#             x2 = self.linear2(self.dropout1(self.activation(self.linear1(x))))
    ...
    def forward(self, x, x2=None):
        ...
        if x2 is not None:
            ...
            if self.seq_pool:
                ...
                x_x2_fusion = torch.matmul(F.softmax(self.attention_pool(x_x2_fusion), dim=1).transpose(-1, -2), x_x2_fusion).squeeze(-2)
            else:
                x2 = x2[:, 0]
                x2 = x_x2_fusion[:, 0]
